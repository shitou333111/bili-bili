...
(function () {
  ...
  var CONFIG = {
    ...
    // —— 翻牌夺宝（唤灵契约）默认配置 ——
    // 玩法：9 张倒扣卡牌，7 张兽魂涌动（flip_result 1..7）+ 2 张灵兽逃脱（flip_result 8..9）。
    // 翻出凶牌立即结束仅获安慰奖（初遇灵石）；可主动退出按已翻出数量结算；翻出 7 张自动结束获最高奖励。
    card_config_id: 10,
    card_act_id: 110687,
    card_act_name: "唤灵契约",
    card_right_num: 7,
    ...
    // 已翻出 n 张兽魂涌动（n=0..6）后，再翻 1 张的价格（分 = 电池×100）。翻出 7 张后无价格。
    card_price_schedule: [5000, 11000, 17000, 31500, 62000, 102500, 203000],
    // 已翻出 n 张兽魂涌动（0..7）对应奖励（value 单位为分）。
  ...
  if (CONFIG.local_image_base) {
    ...
    var _cards = CONFIG.card_rewards || [];
    for (var _ci = 0; _ci < _cards.length; _ci++) {
      if (_cards[_ci] && _cards[_ci].img) _cards[_ci].img = resLocalUrl(_cards[_ci].img);
    }
  }
  ...
  (function () {
    try {
      ...
      if (saved) {
        ...
        if (!parsed || parsed._v !== LING_STATE_VERSION) {
          // 仅玲珑宝斋活动页打印，避免其他活动页（如翻牌夺宝）出现无关日志
          if (algType() === "linglong-open-box") console.log("[LING-MOCK] 检测到旧版本状态，清空重新初始化 (old_v=" + (parsed && parsed._v) + ")");
    ...
    if (algType() === "linglong-open-box") console.log("[LING-MOCK] shim loaded, state_v=" + LING_STATE_VERSION + ", items=" + (CONFIG.item_levels ? CONFIG.item_levels.length : 0));
  })();
  ...
  // ===== 翻牌夺宝（唤灵契约）算法 =====
  // 玩法：9 张倒扣卡牌，7 张兽魂涌动（flip_result 1..7）+ 2 张灵兽逃脱（flip_result 8..9）。
  // 价格随已翻出的兽魂涌动数量递增（card_price_schedule，单位为分）；
  // 翻出凶牌立即结束仅获安慰奖（初遇灵石）；可主动退出按已翻出数量结算（card_rewards）；
  // 翻出 7 张自动结束获最高奖励。
  // 接口（与真实 /cardplay/* 一致）：
  //   HalfInit(GET)：页面初始化配置（样式/文案，下方 makeCardHalfInit 为抓包原文）
  //   StartGame(GET)：新开一局（洗牌 9 张：7 好 + 2 坏），返回首翻价格 flip_card_price
  //   FlipCard(GET)：翻一张卡（随机未翻位置；返回 is_game_over/flip_result/pos/card_map/奖励档位）
  //   Exit（URL 含 Exit/End/Settle/Finish/Quit/Out 段名）：主动退出，按已翻出数量结算
  //   PlayRecord(GET)：获奖记录（mock 空）
  // 价格/价值单位：内部分（电池×100）；card_price_schedule/card_rewards 均为分，与真实接口一致。
  var CARD_STATE_VERSION = 1;
  var cardState = { _v: CARD_STATE_VERSION, game_id: 0, deck: [], flips: {}, right: 0 };
  // "我要开无双"模式：开启后翻牌强制兽魂涌动（好牌），直接冲最高奖励
  var cardUnlimited = false;
  // 本次会话累计获得的礼物总价值（电池），用于 shell 页面 badge 显示
  var cardTotalGiftValue = 0;
  (function () {
    try {
      var saved = localStorage.getItem("bili_activity_card");
      if (saved) {
        var parsed = JSON.parse(saved);
        if (parsed && parsed._v === CARD_STATE_VERSION) {
          cardState = parsed;
        } else {
          localStorage.removeItem("bili_activity_card");
        }
      }
    } catch (e) {}
  })();
  function saveCardState() {
    try {
      cardState._v = CARD_STATE_VERSION;
      localStorage.setItem("bili_activity_card", JSON.stringify(cardState));
    } catch (e) {}
  }
  function resetCardState() {
    cardState = { _v: CARD_STATE_VERSION, game_id: 0, deck: [], flips: {}, right: 0 };
    saveCardState();
  }
  function cardIsActive() {
    return cardState.game_id > 0;
  }
  // 洗牌：9 张卡 → 7 张兽魂涌动（值 1..7 各 1 张）+ 2 张灵兽逃脱（值 8..9 各 1 张）
  function cardShuffleDeck() {
    var deck = [];
    for (var i = 1; i <= 7; i++) deck.push(i);
    deck.push(8, 9);
  ...
  // 已翻出 right 张兽魂涌动后，再翻 1 张的价格（分）
  function cardPriceOf(right) {
    var sched = CONFIG.card_price_schedule || [5000, 11000, 17000, 31500, 62000, 102500, 203000];
    var idx = right;
    if (idx < 0) idx = 0;
    if (idx >= sched.length) idx = sched.length - 1;
    return sched[idx];
  }
  // 已翻出 n 张兽魂涌动（0..7）对应奖励
  function cardRewardFor(n) {
    var rewards = CONFIG.card_rewards || [];
    return rewards[n] || null;
  }
  function makeCardCarousel() {
    var pool = CONFIG.card_carousel_pool || [];
    if (!pool.length) return [];
    var list = [];
    for (var i = 0; i < 10; i++) {
      var it = pool[randInt(0, pool.length - 1)];
      list.push({
        user_name: it.user_name || "模拟用户",
        card_num: it.card_num || 7,
        reward_name: it.reward_name || "虚天鼎曜",
      });
    }
    return list;
  }
  // 结算通知：累计礼物总价值并通知 shell 页面工具条（与 resonance 的 res-total-value 同机制）
  function cardNotifyPrize(prize) {
    if (!prize || !prize.value) return;
    cardTotalGiftValue += Math.round(prize.value / 100);
    var cost = Math.round((10000000 - batteryBalance) / 100);
    try { window.parent.postMessage({ type: "res-total-value", value: cardTotalGiftValue, cost: cost }, "*"); } catch (e) {}
  }
  // 页面初始化：必须返回完整 style_config_map（背景/卡背/卡面/按钮等全部 UI 资源），
  // 否则页面缺资源无法完整加载。下方为抓包到的真实响应原文（config_id=10, act_id=110687）。
  function makeCardHalfInit() {
    var d = {
      code: 0,
      message: "OK",
      ttl: 1,
      data: {
        config_id: CONFIG.card_config_id || 10,
        act_id: CONFIG.card_act_id || 110687,
        act_name: CONFIG.card_act_name || "唤灵契约",
        act_status: 1,
        timestamp: now(),
        start_time: CONFIG.start_time || 1789531200,
        end_time: CONFIG.end_time || 1790006399,
        right_card_num: CONFIG.card_right_num || 7,
        wrong_card_num: CONFIG.card_wrong_num || 2,
        max_draw_right_card_num: CONFIG.card_max_draw_right || 7,
        right_card_name: "正确牌",
        carousel_word_id: 4153,
        word_map: {
          play_carousel_word_1: "翻出灵兽逃脱则本局游戏结束，且仅获得安慰奖",
          play_carousel_word_2: "再翻出1张兽魂涌动，可获得#{next_reward}",
          play_not_start_word: "点击下方按钮，开启游戏",
          room_dm_word: "凡尘问道：恭喜用户#{user_name}在唤灵契约卡牌玩法中获得#{reward_name}",
          top_word_desc: "获取的礼物仅能向#{anchor_face}#{anchor_name}赠送",
          win_broadcast_word: "恭喜#{user_name}翻出#{card_num}张兽魂涌动获得#{reward_name}",
        },
        color_map: {
          nickname_color: "#F5FFFF",
          pos_bg_color: "#FFD0B2",
          word_default_color: "#1F5C57",
          word_desc_color: "#736147",
          word_high_light_color: "#FF4400",
        },
        rule_list: [
          { id: 4153, title: "玩法规则" },
          { id: 3773, title: "其他规则" },
        ...
        style_config_map: {
          anchor_card_arrow: "https://i0.hdslb.com/bfs/live/45159bae0d32de21e82e9304e30aed5fa3e5c9fe.png",
          anchor_card_bg: "https://i0.hdslb.com/bfs/live/f5b67ffb1840cc24a0ee9fc514caf17a49de9536.png",
          bad_title: "https://i0.hdslb.com/bfs/live/f4e2ba18507d4924425e22c2aa8d51906677bb34.png",
          battery_icon: "https://i0.hdslb.com/bfs/live/ebe9144f0ace29615e8a18a9ade434f25ed6c4dc.png",
          btn_cancel: "https://i0.hdslb.com/bfs/live/104cbdf6d8ab4ff557492dbd6f258a224985c0c4.png",
          btn_confirm: "https://i0.hdslb.com/bfs/live/0ad5598bc8eaa0b274f9e6d2428eb3043f46ef32.png",
          card: "https://i0.hdslb.com/bfs/live/af7e6f6fab5dc61be6b65741efc16b28c79b73ca.png",
          card_1: "https://i0.hdslb.com/bfs/live/5e27f73e3d0b9ba2a1b2eb19d733ae1b548b5d14.png",
          card_2: "https://i0.hdslb.com/bfs/live/5e27f73e3d0b9ba2a1b2eb19d733ae1b548b5d14.png",
          card_3: "https://i0.hdslb.com/bfs/live/5e27f73e3d0b9ba2a1b2eb19d733ae1b548b5d14.png",
          card_4: "https://i0.hdslb.com/bfs/live/5e27f73e3d0b9ba2a1b2eb19d733ae1b548b5d14.png",
          card_5: "https://i0.hdslb.com/bfs/live/5e27f73e3d0b9ba2a1b2eb19d733ae1b548b5d14.png",
          card_6: "https://i0.hdslb.com/bfs/live/5e27f73e3d0b9ba2a1b2eb19d733ae1b548b5d14.png",
          card_7: "https://i0.hdslb.com/bfs/live/5e27f73e3d0b9ba2a1b2eb19d733ae1b548b5d14.png",
          card_8: "https://i0.hdslb.com/bfs/live/2dcbb5352b189288fac2b24c522f2fcacb484131.png",
          card_9: "https://i0.hdslb.com/bfs/live/2dcbb5352b189288fac2b24c522f2fcacb484131.png",
          card_checked: "https://i0.hdslb.com/bfs/live/ef2172020803ee8cd52ecf78f9674effcc0459c6.png",
          checkbox_checked: "https://i0.hdslb.com/bfs/live/69ead981891aee137ef58ab99de3947772e180fe.png",
          checkbox_unchecked: "https://i0.hdslb.com/bfs/live/96324e3294ff893edfb524fb1d6c13fa82f29f06.png",
          corner: "https://i0.hdslb.com/bfs/live/fe927e4bd50688fede70f80c734fe986a9aac4f5.png",
          drawer_bg: "https://i0.hdslb.com/bfs/live/de45f337e18d7bae6a20d24c477a15d794464418.png",
          drawer_close: "https://i0.hdslb.com/bfs/live/1d45988f0f7b943aa3e443037972b04b30965f65.png",
          exit_record: "https://i0.hdslb.com/bfs/live/99785a2b87961b96aa2ffb9e10c2aedac9f3caf4.png",
          fail: "https://i0.hdslb.com/bfs/live/0ad601541e8fa48c0843080828cf4ebadf0d81db.mp4",
          fail_json: "https://i0.hdslb.com/bfs/live/97edd66eb1d2dc00a102afa8d351d63b74450061.json",
          good_title: "https://i0.hdslb.com/bfs/live/a6895189e36df52e6439b5ac91b3dd2b9c5b9511.png",
          kv: "https://i0.hdslb.com/bfs/live/6d668c73e8d346a076cea547aea3e57a32916925.png",
          marquee_bg: "https://i0.hdslb.com/bfs/live/afd4e87b36dedc114b07f16dd91ce97c9b869954.png",
          play_bg: "https://i0.hdslb.com/bfs/live/95af07e1b4f625a23253531d917c32a391d1ace4.png",
          play_btn_1: "https://i0.hdslb.com/bfs/live/701e28a1bdb1b38594ca838b2c95ad022e2f71d4.png",
          play_btn_2: "https://i0.hdslb.com/bfs/live/90afab6cfbf87cd3e6994c6abb7f9d78ce9811a8.png",
          play_btn_3: "https://i0.hdslb.com/bfs/live/90afab6cfbf87cd3e6994c6abb7f9d78ce9811a8.png",
          play_btn_4: "https://i0.hdslb.com/bfs/live/00adc9246097b417bac9035c681ab09f6e0389a0.png",
          pop_bg: "https://i0.hdslb.com/bfs/live/d203647fce32d600ac51118a01f14f1bc6a3f064.png",
          question: "https://i0.hdslb.com/bfs/live/a85fa6813f15ffe19838012d5ca2b5e6b79ef5d7.png",
          record_btn: "https://i0.hdslb.com/bfs/live/51db4e3cbc97e8091599b8731e97cb68f7421446.png",
          record_item_bg_high: "https://i0.hdslb.com/bfs/live/6df6f3197e7c1767fc9d743ce714ab5beeb7387e.png",
          record_item_bg_low: "https://i0.hdslb.com/bfs/live/7588fdbcc292a24aedc184697be3508c0126daa3.png",
          record_title: "https://i0.hdslb.com/bfs/live/9126a34d2fa18fa15f4a3b6ba0e8bafdfbe05b0f.png",
          right: "https://i0.hdslb.com/bfs/live/a0098f3ccead604f51b8adf9c4f30ff9c987d545.svga",
          rule_arrow: "https://i0.hdslb.com/bfs/live/976fa9d84ab50834659bc09169ee08b90cdb4579.png",
          rule_btn: "https://i0.hdslb.com/bfs/live/c15934ae815eb7c38160e6cbcb89e4c9b1787f34.png",
          rule_header_bg: "https://i0.hdslb.com/bfs/live/99ce0df6aeeaac8743d39102c8d304978eb9d71d.png",
          rule_title: "https://i0.hdslb.com/bfs/live/f7f1ff5d5fcf9d62841d27e73911e7632cf8c585.png",
          very_good_title: "https://i0.hdslb.com/bfs/live/c8228aff9f136ccad48a4675faff4545ae33ff78.png",
          win: "https://i0.hdslb.com/bfs/live/55449713589f7b8afcdaf3ee543ace5196992994.mp4",
          win_json: "https://i0.hdslb.com/bfs/live/97edd66eb1d2dc00a102afa8d351d63b74450061.json",
          wrong: "https://i0.hdslb.com/bfs/live/a0098f3ccead604f51b8adf9c4f30ff9c987d545.svga",
        },
      },
    };
    // 镜像模式：style_config_map 全部 CDN 图片 URL 改写为本地镜像同源路径（见 resLocalUrl 注释）。
  ...
  // 当前局状态查询（未知查询接口兜底，不结束游戏）
  function makeCardGameState() {
    var active = cardIsActive();
    var right = active ? cardState.right : 0;
    var cur = active ? cardRewardFor(right) : null;
    var next = active ? cardRewardFor(right + 1) : null;
    var d = {
      game_id: active ? cardState.game_id : 0,
      flip_card_price: active ? cardPriceOf(right) : 0,
      card_map: active ? cardState.flips : {},
      right_card_num: right,
      is_game_over: false,
      current_reward: cur && cur.name ? cur.name : "",
      current_reward_value: cur ? cur.value : 0,
      next_reward: next ? next.name : "",
      next_reward_value: next ? next.value : 0,
      carousels: makeCardCarousel(),
    };
    return { code: 0, message: "OK", ttl: 1, data: d };
  }
  // 新开一局：洗牌 + 开 game_id，返回首翻价格（与真实 StartGame 响应一致，不扣费不翻牌）
  function makeCardStartGame() {
    cardState.game_id = randInt(30000, 99999);
    cardState.deck = cardShuffleDeck();
    cardState.flips = {};
    cardState.right = 0;
    saveCardState();
    var next = cardRewardFor(1);
    var d = {
      game_id: cardState.game_id,
      flip_card_price: cardPriceOf(0),
      next_reward: next ? next.name : "",
      next_reward_value: next ? next.value : 0,
    };
    console.log("[CARD-MOCK] StartGame game_id=" + cardState.game_id + " price=" + d.flip_card_price);
    return { code: 0, message: "OK", ttl: 1, data: d };
  }
  // 翻一张卡：扣 price[right]，翻开用户点击的那张（请求携带的 pos）；凶牌 → 立即结束仅获安慰奖；
  // 好牌 → right++，翻出 7 张自动结束获最高奖励；否则返回当前/下一档奖励与下次翻牌价格。
  // 位置以 1 开始计数（真实请求点第几张就传 pos=几，响应 card_map 的 key 即 pos 值）；
  // 请求未携带 pos（"任意翻一张"）时随机选一个未翻位置。
  function makeCardFlip(p) {
    p = p || {};
    if (!cardIsActive()) makeCardStartGame();
    console.log("[CARD-MOCK] Flip params=" + JSON.stringify(p));
    try { localStorage.setItem("bili_activity_card_last_params", JSON.stringify(p)); } catch (e) {}
    var paid = cardPriceOf(cardState.right);
    batteryBalance = Math.max(0, batteryBalance - paid);
    var positions = [];
    for (var _fp = 1; _fp <= 9; _fp++) {
      if (cardState.flips[_fp] === undefined) positions.push(_fp);
    }
    // 解析用户点击的位置（1 基 1-9）：真实请求字段名即 "pos"，点第几张就传第几张，只认它一个。
    // 请求未携带 pos（如"任意翻一张"按钮）或 pos 非法时，随机翻开一张未翻的牌。
    // 注意：pos 合法即接受，不要求未翻过——若该位置已被翻（如先前"任意翻一张"随机翻到过），
    // 按已有结果返回，避免"点已翻的牌却随机翻别处"。
    var wanted = -1;
    if (p.pos !== undefined && p.pos !== null && p.pos !== "") {
      var _pv = parseInt(p.pos, 10);
      if (!isNaN(_pv) && _pv >= 1 && _pv <= 9) wanted = _pv;
    }
    var pos = wanted >= 1 ? wanted : positions.length ? positions[randInt(0, positions.length - 1)] : -1;
    var repeat = pos >= 1 && cardState.flips[pos] !== undefined;
    // 抽取卡面结果：正常模式按剩余牌堆抽；无双模式强制好牌；已翻位置直接取已有结果
    var result;
    if (repeat) {
      result = cardState.flips[pos];
    } else if (cardUnlimited) {
      var goodLeft = [];
      for (var g = 0; g < cardState.deck.length; g++) {
        if (cardState.deck[g] <= 7) goodLeft.push(cardState.deck[g]);
      }
      result = goodLeft[randInt(0, goodLeft.length - 1)];
    } else {
      result = cardState.deck[randInt(0, cardState.deck.length - 1)];
    }
    if (!repeat) {
      cardState.deck.splice(cardState.deck.indexOf(result), 1);
      cardState.flips[pos] = result;
    }
    var d = {
      is_game_over: false,
      flip_card_price: 0,
      card_map: cardState.flips,
      flip_result: result,
      pos: pos,
      reward_name: "",
      reward_img: "",
      ...
      next_reward: "",
      next_reward_value: 0,
      right_card_num: cardState.right,
      carousels: makeCardCarousel(),
    };
    if (repeat) {
      // 已翻位置重复点击：仅返回当前局进度，不重复抽取/计数/结束
      var cur0 = cardRewardFor(cardState.right);
      var next0 = cardRewardFor(cardState.right + 1);
      d.flip_card_price = cardPriceOf(cardState.right);
      d.right_card_num = cardState.right;
      d.current_reward = cur0 ? cur0.name : "";
      d.current_reward_value = cur0 ? cur0.value : 0;
      d.next_reward = next0 ? next0.name : "";
      d.next_reward_value = next0 ? next0.value : 0;
      console.log("[CARD-MOCK] Flip REPEAT pos=" + pos + " result=" + result);
      return { code: 0, message: "OK", ttl: 1, data: d };
    }
    if (result <= 7) {
      cardState.right++;
      if (cardState.right >= (CONFIG.card_max_draw_right || 7)) {
        // 翻出 7 张 → 最高奖励，自动结束
        var top = cardRewardFor(cardState.right);
        var totalRight = cardState.right;
        d.is_game_over = true;
        d.flip_card_price = 0;
        d.right_card_num = totalRight;
        d.reward_name = top ? top.name : "";
        d.reward_img = top ? top.img : "";
        d.reward_value = top ? top.value : 0;
        cardNotifyPrize(top);
        resetCardState();
        console.log("[CARD-MOCK] Flip GOOD #" + totalRight + " pos=" + pos + " reward=" + (top && top.name));
      } else {
        var cur = cardRewardFor(cardState.right);
        var next = cardRewardFor(cardState.right + 1);
        d.flip_card_price = cardPriceOf(cardState.right);
        d.right_card_num = cardState.right;
        d.current_reward = cur ? cur.name : "";
        d.current_reward_value = cur ? cur.value : 0;
        d.next_reward = next ? next.name : "";
        d.next_reward_value = next ? next.value : 0;
        saveCardState();
        console.log("[CARD-MOCK] Flip GOOD #" + cardState.right + " pos=" + pos + " result=" + result);
      }
    } else {
      // 凶牌 → 立即结束，仅获安慰奖（初遇灵石 50 电池）
      var cons = cardRewardFor(1) || { name: "", value: 0, img: "" };
      d.is_game_over = true;
      d.flip_card_price = 0;
      d.right_card_num = 0;
      d.reward_name = cons.name;
      d.reward_img = cons.img;
      d.reward_value = cons.value;
      cardNotifyPrize(cons);
      resetCardState();
      console.log("[CARD-MOCK] Flip BAD pos=" + pos + " result=" + result + " consolation=" + cons.name);
    }
    return { code: 0, message: "OK", ttl: 1, data: d };
  }
  // 主动退出：按已翻出的兽魂涌动数量结算并清局。
  // 响应字段与真实 ExitGame 接口严格一致（data 仅 reward_name/reward_img/reward_value/right_card_num 四字段），
  // 奖励按已翻出数量动态返回（right_card_num → card_rewards 对应档位）。
  function makeCardExit() {
    var right = cardState.right;
    var prize = right >= 1 ? cardRewardFor(right) : null;
    var d = {
      reward_name: prize ? prize.name : "",
      reward_img: prize ? prize.img : "",
      reward_value: prize ? prize.value : 0,
      right_card_num: right,
    };
    cardNotifyPrize(prize);
    resetCardState();
    console.log("[CARD-MOCK] Exit right=" + right + " prize=" + (prize && prize.name));
    return { code: 0, message: "OK", ttl: 1, data: d };
  }
  // 获奖记录：本地 mock 空列表
  function makeCardPlayRecord() {
    return { code: 0, message: "OK", ttl: 1, data: { list: [] } };
  }
  ...
  function handleRequest(url, body) {
    ...
    // —— 翻牌夺宝（唤灵契约）算法：拦截 cardplay/* 玩法接口 + 登录态/钱包；其余放行。 ——
    // URL 兜底识别：即使注入的 algorithmType 缺失/过期，只要命中 /cardplay/ 接口也强制本地 mock，
    // 避免误发真实请求导致"未登录/扣费"。接口对应关系见 makeCard* 注释。
    if (algType() === "card-flip" || /\/cardplay\//i.test(url)) {
      // 真实请求把 pos（点击位置）放在 query 或 POST body；合并两者，保证按用户点击的牌返回。
      var cq = parseQuery(url);
      for (var _fk in params) {
        if (params[_fk] !== undefined && cq[_fk] === undefined) cq[_fk] = params[_fk];
      }
      if (/\/cardplay\/HalfInit/i.test(url)) return makeCardHalfInit();
      if (/\/cardplay\/StartGame/i.test(url)) return makeCardStartGame();
      if (/\/cardplay\/FlipCard/i.test(url)) return makeCardFlip(cq);
      if (/\/cardplay\/ExitGame/i.test(url)) return makeCardExit();
      if (/(Record|History)/i.test(url)) return makeCardPlayRecord();
      if (/x\/web-interface\/nav/i.test(url)) return fakeLoginNav();
      if (/xlive\/revenue\/v1\/wallet\/myWallet/i.test(url)) return fakeWallet();
      return makeCardGameState();
    }
  ...
  function shouldMock(url) {
    ...
    // 翻牌夺宝（唤灵契约）算法：拦截 cardplay/* 玩法接口 + 登录态/钱包，其余放行真实服务器。
    if (algType() === "card-flip" || /\/cardplay\//i.test(url)) {
      return (
        /\/cardplay\//i.test(url) ||
        isLoginOrWallet(url) ||
