  ...
  createClass(ParticleEffectButton, [{
    ...
    value: function _addParticle(opts) {
      ...
      this._particles.push({
        ...
        life: 0,
        // 消散时粒子存活帧数与 badge 滑出进度对齐（按创建时的剩余进度），使粒子恰好随
        // badge 完全滑出(progress=100)时散尽，避免粒子提前死亡触发状态翻转导致 badge
        // 未滑完就闪现消失（原库 hiding 提前 20~40 帧死亡）。
        death: status === 'hiding' && opts.progress != null ? Math.max(1, frames * (1 - opts.progress)) : frames,
        speed: _speed,
        size: _size
      });
    }
